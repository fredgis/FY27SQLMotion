...
description: "Guided advisor that recommends the best path to migrate a SQL Server environment to Azure. Conducts a short structured interview (target, source version, downtime tolerance, feature dependencies, compliance, ancillary services), then scores and recommends a target (SQL VM / AVS / SQL MI / SQL DB / Fabric SQL DB / Arc SQL MI / container or Arc in-place), a migration method (MI Link / LRS / backup-restore / DAG / DMS / replication / BACPAC / Fabric Migration Assistant), a downtime class, blockers + remediations, cost levers (AHB / ESU), and the right Microsoft program (Cloud Accelerate Factory / SQL in a Day). Trigger whenever the user wants to migrate or modernize SQL Server to Azure, asks for the best/recommended migration path or target, mentions 'migrer SQL Server', 'migrate SQL Server', 'SQL to Azure', 'Azure SQL Managed Instance vs SQL DB', 'chemin de migration', 'SQL in a Day', or wants help choosing a migration tool. Knowledge base: docs/sql-server-to-azure-migration.md in github.com/fredgis/FY27SQLMotion (verified June 2026)."
license: MIT
---
